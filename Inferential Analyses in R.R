# R Codes
# SETTING DIRECTORY
if(dir.exists("D:\\Tutorials\\R\\R_Codes")){
  setwd("D:\\Tutorials\\R\\R_Codes")
}else{
  dir.create("D:\\Tutorials\\R\\R_Codes")
  setwd("D:\\Tutorials\\R\\R_Codes")
}
...
library(haven)
STIData <- read_stata("STIData.dta")
...
# 1.i) Pearson's r
cor.test(STIData$Weight, STIData$Height)

with(STIData, cor(Weight, Height, method = "pearson"))
with(STIData, cor.test(Height, Weight, method = "pearson")) #Gives the p-value 

library(ggplot2)
ggplot(STIData, aes(x = Height, y = Weight)) + 
  geom_point(position = "jitter") + 
  geom_smooth(method = "lm", se = FALSE)+
  annotate("text", label = "R = 0.34", x = 145, y = 95, size = 4, colour = "black")+
...
# 2.ii) Kendall's tau ----
with(STIData, cor(Weight, Height, method = "kendall"))
with(STIData, cor.test(Weight, Height, method = "kendall")) # Based on ranks, cannot compute exact p-value with ties
...
write.csv(ctable, "Multinomial Logit.csv")
...
summary(TW_ANOVANoInteraction)
coefficients(TW_ANOVANoInteraction)
TukeyHSD(TW_ANOVANoInteraction)
plot(TukeyHSD(TW_ANOVANoInteraction))
